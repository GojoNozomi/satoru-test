...
// ===== 已修改：持久化主动来信的下次到期时间 =====
// 页面被 iOS 挂起或被系统回收后，内存中的 setTimeout 不可靠；时间戳可以在恢复时补偿。
function getEnvelopeAutoSendNextAtKey() {
    try {
        return typeof getStorageKey === 'function' ? getStorageKey('envelopeAutoSendNextAt') : 'CHAT_APP_V3_envelopeAutoSendNextAt';
    } catch (e) {
        return 'CHAT_APP_V3_envelopeAutoSendNextAt';
    }
}

function readEnvelopeAutoSendNextAt() {
    try { return Number(localStorage.getItem(getEnvelopeAutoSendNextAtKey()) || 0); } catch (e) { return 0; }
}

function writeEnvelopeAutoSendNextAt(value) {
    try {
        const key = getEnvelopeAutoSendNextAtKey();
        if (value) localStorage.setItem(key, String(value));
        else localStorage.removeItem(key);
    } catch (e) {}
}

function manageEnvelopeAutoSendTimer(forceReschedule) {
    // 清除现有定时器
    ...
    if (!settings.envelopeAutoSendEnabled) {
        writeEnvelopeAutoSendNextAt(0);
        return;
    }

    if (forceReschedule) writeEnvelopeAutoSendNextAt(0);

    // 计算或读取下次写信时间，避免每次页面恢复都重新随机导致计划被推迟。
    ...
    let nextAt = readEnvelopeAutoSendNextAt();
    if (!Number.isFinite(nextAt) || nextAt <= 0) {
        const randomMs = Math.random() * Math.max(0, maxMs - minMs) + minMs;
        nextAt = Date.now() + randomMs;
        writeEnvelopeAutoSendNextAt(nextAt);
    }

    const scheduledAt = nextAt;
    envelopeAutoSendTimer = setTimeout(() => {
        if (!settings.envelopeAutoSendEnabled || readEnvelopeAutoSendNextAt() !== scheduledAt) return;
        writeEnvelopeAutoSendNextAt(0);
